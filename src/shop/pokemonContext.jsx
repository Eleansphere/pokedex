...
export default function PokemonContextProvider({ children }) {
    ...
    function handleSearch() {
      if (inputValue.current.value.trim() === '') {
        ...
        pokemonList.pokemons.find(
          (pokemon) =>
            inputValue.current.value.trim().toLowerCase() ===
            pokemon.name
        )
      ) {
        setDataError(true);
  ...
  let currentIndex = pokemonList.pokemons.findIndex(
    (index) => index.id === pokemonList.selectedPokemonId
  );
    ...
    if (currentIndex < pokemonList.pokemons.length - 1) {
      setPokemonList((prevState) => {
        return {
          ...
          selectedPokemonId:
            pokemonList.pokemons[
              (currentIndex + 1) % pokemonList.pokemons.length
            ].id,
        };
      });
    }
    ...
    if (currentIndex > 0) {
      setPokemonList((prevState) => {
        return {
          ...
          selectedPokemonId:
            pokemonList.pokemons[
              (currentIndex + pokemonList.pokemons.length - 1) %
